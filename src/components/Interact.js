...
const initialAnswers = [
  {
    intent: "Group 5 Cloud Assistant",
    description: "The Group 5 Cloud Assistant is an AI-powered tool built to help users configure and manage their AWS accounts using natural language. Developed by Group 5, this assistant utilizes advanced NLU (Natural Language Understanding) and a flexible serverless backend to handle both simple and complex AWS configurations on your behalf.",
    data: {}
  },
];
  ...
  useEffect(() => {
    async function loadConversationDetails() {
      ...
      if (result) {
        ...
        // Get utterances
        const utterances =
          conversation.utterances && conversation.utterances.items
        ...
        const validUtterances = utterances.filter(
          (u) =>
            u &&
            typeof u.author === "string" &&
            typeof u.text === "string"
        );


        // Set dialogs state
        let dialogsArray = initialDialogs.concat(validUtterances)
        ...
        <Box sx={{ overflow: "auto", height: "100vh" }}>
          <List sx={{ height: "calc(100% - 73px)", overflowY: "scroll" }}>
            {dialogs.map((dialog, index) =>
              dialog && dialog.author && dialog.text ? (
                <span key={index}>
                  {dialog.author === "AWS" ? (
                    <AwsDialogItem text={dialog.text} />
                  ) : (
                    <MeDialogItem
                      text={dialog.text}
                      reusePreviousDialog={reusePreviousDialog}
          ...
          {answers.map((answer, index) => (
            <>
              <ResponseCard
                ...
                cardDescription={answer.description}
                data={answer.data}
              />
            </>
          ))}
